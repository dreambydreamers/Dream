...
- **Source of truth:** `ExploreMediaRepository.shared` merges recent `dream_videos` (cover + update videos) with `dream_photo_updates` (photo updates), enriches them with dream/profile/stats/timeline data, and sorts newest-first.
...
- **`ExploreMediaRepository.shared`** — `@MainActor ObservableObject` for Explore/Profile update media. It fetches videos and photos independently, enriches them with dream/profile context, maps video rows into `Dream` values for playback/share/detail, and keeps `items` for the Explore grid.
